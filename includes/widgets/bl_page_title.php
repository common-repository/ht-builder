...
class Bl_Page_Title_ELement extends Widget_Base {
    ...
    protected function render( $instance = [] ) {
        ...
        if( Elementor::instance()->editor->is_edit_mode() ){
            echo sprintf( '<%1$s class="htpage-title">' . esc_html__('Page Title', 'ht-builder' ). '</%1$s>', esc_attr( $title_tag ) );
        }else{
            echo sprintf( '<%1$s class="htpage-title">%2$s</%1$s>' , esc_attr( $title_tag ), esc_html( wp_title( '', false ) ) );
        }
    }
