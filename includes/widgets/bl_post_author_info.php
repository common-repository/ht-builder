...
class Bl_Post_Author_Info_ELement extends Widget_Base {
    ...
    protected function render( $instance = [] ) {
        ...
        $title_tag = htbuilder_validate_html_tag( $settings['name_html_tag'] );
        // Aavater image
                ...
                <div class="htauthor-info">
                    <?php
                        if( $settings['show_name'] == 'yes' ){
                            if( !empty( $author_link ) ){
                                echo sprintf( '<a href="%1$s" target="_blank"><%2$s class="htauthor-name">%3$s</%2$s></a>', esc_url( $author_link ) , esc_attr( $title_tag ), esc_html( $author_info['display_name'] ) );
                            }else{
                                echo sprintf( '<%1$s class="htauthor-name">%2$s</%1$s>', esc_attr( $title_tag ), esc_html( $author_info['display_name'] )  );
                            }
                        }
